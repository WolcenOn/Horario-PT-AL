...
export function renderProfessionals(root, { state, onEdit, onDelete }) {
  ...
  root.innerHTML = `<section class="card"><div class="card-header"><div><h2>Profesorado del centro</h2><small>Perfiles, tutorías, materias habilitadas, docencia fijada, funciones y disponibilidad.</small></div><span class="badge badge-neutral">${state.professionals.length} profesionales</span></div><div class="table-wrap"><table><thead><tr><th>Profesional</th><th>Tipo</th><th>Tutoría</th><th>Puede impartir</th><th>Docencia fijada</th><th>Funciones</th><th>Capacidad semanal</th><th>Otros centros</th><th>Acciones</th></tr></thead><tbody>${rows || `<tr><td colspan="9"><div class="empty-state"><strong>No hay profesorado</strong>Utiliza “Nuevo profesor” para comenzar.</div></td></tr>`}</tbody></table></div></section>`;
...
export function openProfessionalForm(professional, { state, onSave }) {
  ...
  const permissionSubjects = [...new Set([...FIXED_SUBJECT_NAMES, ...(current.allowedSubjects || [])])];
  ...
  showModal({
    ...
    bodyHtml:`<div class="form-grid">
      ...
      <div class="form-field full"><fieldset><legend>Presencia mínima deseada con su tutoría</legend><div class="duration-pair"><div class="form-field"><label>Horas</label><input name="minTutorHours" type="number" min="0" value="${minTutor.hours}"></div><div class="form-field"><label>Minutos</label><input name="minTutorMinutes" type="number" min="0" max="59" value="${minTutor.minutes}"></div></div><p class="field-hint">Por ahora se usa como dato de planificación; el solver de reparto lo incorporará en la siguiente fase.</p></fieldset></div>
      <fieldset class="full"><legend>Materias que puede impartir</legend><div class="subject-permission-grid">${permissionsHtml}</div><p class="field-hint">Marca habilitaciones reales, no solo las materias que actualmente tiene asignadas. Las asignaciones fijadas se añaden automáticamente a esta lista.</p></fieldset>
      <fieldset class="full"><legend>Asignaturas y clases ya fijadas</legend><div id="teachingAssignments" class="teaching-assignment-list"></div><button class="button" type="button" data-add-teaching>+ Fijar asignatura / clase</button><p class="field-hint">Una asignación aquí significa “este docente debe impartir esta materia a este grupo”. Déjala sin fijar si quieres que el futuro solver pueda decidir.</p></fieldset>
