  ...
  root.innerHTML = `<div class="integration-view">
    ...
    <section class="card">
      ...
      <div class="card-body">
        ...
        <div class="button-row integration-sync-actions">
          ...
          <span class="field-hint">La sincronización sigue siendo de centro completo en esta fase. Curso académico y escenario ya se guardan como contexto, pero todavía no particionan estos datos remotos.</span>
        </div>
      </div>
    </section>
    ...
    <section class="card integration-roadmap">
      ...
      <div class="card-body integration-roadmap-grid">
        ...
        <div class="is-done"><b>3</b><span><strong>Curso académico y escenarios</strong><small>Contexto persistente para preparar cursos y alternativas sin sobrescribir el anterior.</small></span></div>
        <div><b>4</b><span><strong>Sincronización segura</strong><small>Versionado multiusuario, autorización y autenticación real.</small></span></div>
      </div>
    </section>
  </div>`;
...
function renderAcademicContextCard(settings, configured, context) {
  ...
  return `<section class="card">
    <div class="card-header"><div><h2>Curso académico y escenario</h2><small>Este contexto se guarda en el navegador y será la base para separar posteriormente plantilla, matrícula y horarios por curso.</small></div><span class="badge ${selectedYearId && selectedScenarioId ? 'badge-success' : 'badge-neutral'}">${selectedYearId && selectedScenarioId ? 'Contexto completo' : 'Pendiente'}</span></div>
